...
add_filter(
	'set_screen_option_wpselzy_forms_per_page',
	function ( $result, $option, $value ) {
		$wpselzy_screens = array(
			'wpselzy_forms_per_page',
		);

		if ( in_array( $option, $wpselzy_screens ) ) {
			$result = $value;
		}
...
function wpselzy_load_form_admin() {
	...
	if ( 'save' == $action ) {
		...
		if ( ! current_user_can( 'wpselzy_edit_form', $id ) ) {
			wp_die(
				__( "You're not allowed to edit this element.", 'selzy' )
			);
		}
		...
		$args['form'] = isset( $_POST['wpselzy-form'] )
			? $_POST['wpselzy-form'] : '';

		$args['contact_list'] = isset( $_POST['wpselzy-contact-list'] )
			? $_POST['wpselzy-contact-list'] : '';
	...
	if ( 'copy' == $action ) {
		...
		if ( ! current_user_can( 'wpselzy_edit_form', $id ) ) {
			wp_die(
				__( "You're not allowed to edit this element.", 'wpselzy' )
			);
		}
	...
	if ( 'delete' == $action ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( ! current_user_can( 'wpselzy_delete_form', $post->id() ) ) {
				wp_die(
					__( "You're not allowed to delete this element.", 'selzy' )
				);
			}

			if ( ! $post->delete() ) {
				wp_die( __( "Delete error.", 'selzy' ) );
			}
...
function wpselzy_admin_management_page() {
	...
	?>
    <div class="wrap" id="wpselzy-form-list-table">

        <h1 class="wp-heading-inline"><?php
			echo esc_html( __( 'Forms', 'selzy' ) );
			?></h1>
        ...
        <form method="get" action="">
			...
			<?php $list_table->search_box( __( 'Search', 'selzy' ), 'wpselzy-form' ); ?>
...
function wpselzy_admin_updated_message( $page, $action, $object ) {
	...
	if ( 'created' == $_REQUEST['message'] ) {
		$updated_message = __( "Form is created.", 'selzy' );
	} elseif ( 'saved' == $_REQUEST['message'] ) {
		$updated_message = __( "Form is saved.", 'selzy' );
	} elseif ( 'deleted' == $_REQUEST['message'] ) {
		$updated_message = __( "Form is deleted.", 'selzy' );
	}
...
function wpselzy_admin_settings_notice() {
	...
	printf(
		...
		sprintf(
		/* translators: 1: Link start tag to the Selzy Settings page, 2: Link closing tag. */
			esc_html__( 'Please provide an API key in %1$splugin settings%2$s to get started with Selzy.',
				'selzy' ),
...
function wpselzy_admin_should_display_settings_notice() {
	...
	return (
		...
		&& (
			$GLOBALS['pagenow'] === 'admin.php' && in_array( filter_input( INPUT_GET, 'page' ), $selzy_pages )
...
function wpselzy_old_wp_version_error( $page, $action, $object ) {
    ...
    <div class="notice notice-warning">
        <p><?php
			echo sprintf(
			/* translators: 1: version of Selzy, 2: version of WordPress, 3: URL */
				__( '<strong>Selzy %1$s требует WordPress %2$s или новее.</strong> Пожалуйста, сначала <a href="%3$s">обновите WordPress</a>.',
					'selzy' ),
				WPSELZY_VERSION,
...
function wpselzy_not_allowed_to_edit( $page, $action, $object ) {
	...
	$message = __( "You're not allowed to edit this form.", 'selzy' );
